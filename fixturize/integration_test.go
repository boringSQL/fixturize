...
// insertOrphans is a test helper that forcibly inserts rows which violate
// referential integrity. Because PostgreSQL would normally reject such rows
// outright, the helper disables foreign key triggers for the duration of the
// transaction by setting session_replication_role to 'replica'. Using a single
// transaction keeps that override neatly scoped and ensures that every
// statement runs against the same pooled connection.
...
// findOrphanResult is a test helper that locates a single orphan result by the
// child table name and the exact, ordered set of foreign key columns. It
// deliberately matches on the columns rather than the constraint name, because
// constraint names are generated by PostgreSQL and are therefore not stable
// enough to hard-code into a test.
...
// TestCheckOrphans is an integration test that verifies CheckOrphans correctly
// detects orphaned rows across single-column and composite foreign keys, while
// reporting zero orphans for foreign keys that are consistently seeded.
func TestCheckOrphans(t *testing.T) {
	// Arrange: reset the database and seed it with a known-good dataset.
	resetDB(t, testDB)
	seedAll(t, testDB)
	ctx := t.Context()

	// Arrange: deliberately insert rows that break referential integrity so
	// that there is something for the check to discover.
	insertOrphans(t, testDB, []string{
		// A single-column FK orphan where owner_id points at a user that does
		// not exist anywhere in the users table.
		`INSERT INTO projects (id, owner_id, name) VALUES (99, 9999, 'Orphan Project')`,
		// Another single-column FK orphan, this time where task_id points at a
		// task that does not exist.
		`INSERT INTO task_comments (id, task_id, body) VALUES (99, 8888, 'Orphan comment')`,
		// A composite FK orphan: the workspace_projects table is empty, so any
		// row inserted here is necessarily orphaned.
		`INSERT INTO project_assignments (id, workspace_id, project_id, assignee) VALUES (1, 1, 1, 'nobody')`,
	})

	// Act: introspect the live schema so the check has constraints to walk.
	schema, err := IntrospectSchema(ctx, testDB)
	if err != nil {
		t.Fatalf("introspect: %v", err)
	}

	// Act: run the orphan check across every table in the schema.
	results, err := CheckOrphans(ctx, testDB, schema, schema.GetTables(), CheckOptions{Samples: 5})
	if err != nil {
		t.Fatalf("check orphans: %v", err)
	}

	// Assert: the three orphan rows inserted above should be the grand total.
	if total := TotalOrphans(results); total != 3 {
		t.Errorf("expected 3 orphaned rows total, got %d", total)
	}

	// Assert: the projects.owner_id foreign key should report its single orphan.
	...
	// Assert: the task_comments.task_id foreign key should report its orphan.
	...
	// Assert: the composite foreign key on project_assignments should likewise
	// report exactly one orphaned row.
	...
	// Assert: a foreign key that was consistently seeded must report zero
	// orphans, proving the check does not raise false positives.
...
// TestCheckOrphansCleanDatabase is an integration test that verifies the orphan
// check reports a perfectly clean result when run against a database that has
// only ever been populated with consistent, referentially-intact data.
func TestCheckOrphansCleanDatabase(t *testing.T) {
	// Arrange: reset the database and seed it with a known-good dataset, taking
	// care not to introduce any orphaned rows this time.
	resetDB(t, testDB)
	seedAll(t, testDB)
	ctx := t.Context()
	...
	// Assert: a healthy database should yield a grand total of zero orphans.
	if total := TotalOrphans(results); total != 0 {
		t.Errorf("expected 0 orphans on a consistently-seeded DB, got %d", total)
	}
	// Assert: the rendered summary should describe the database as satisfied.
